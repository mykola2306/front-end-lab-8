...
function tankPreviewPage(arrTankObj) {
  ...
  for (let i = 0; i < arrTankObj.length; i++) {
    ...
    if (arrTankObj[i].model) {
      ...
      tankFlag.innerHTML = `<span class="tooltipImgText">${arrTankObj[i].country}</span><img alt="${arrTankObj[i].country}" class="imgAutoWidth" src="${arrTankObj[i].country_image}">`;
      tankName.innerHTML = `<span class="tooltipName">${arrTankObj[i].model}</span><span>${arrTankObj[i].level}</span> ${arrTankObj[i].model}`;
...
function tankDetails(tank) {
  ...
  returnToThumbnails.innerHTML = `<a href="#">Back to list view</a>`;
